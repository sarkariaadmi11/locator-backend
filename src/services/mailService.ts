...
// Email registration OTP (PRD §4.1 "First-Time Registration: Phone/Email input → OTP" — the
// canonical registration flow requires OTP verification for either input channel, not just
// phone). Brevo is the chosen delivery vendor; the PRD/TRD names MSG91 for SMS but does not
// name an email vendor, so any transactional email provider satisfies the requirement.
