...
`requestId`, opens on Creator acceptance + GPS validation, closes permanently on `Start Recording` tap (PRD §5.4). `ChatMessage`: `senderId`, `body`, `blocked` (bool — logged even when blocked), `blockReason?`. Retention: 90 days from request close [PRD REVIEW-tagged, confirm before building retention job]. Server-side regex blocking is mandatory (phone numbers, email, WhatsApp/Telegram/Instagram handles, UPI VPAs, URLs) — **do this server-side**, not just client-side, since chat is logged for moderation audit.
...
### `RequestVideo`
`requestId`, storage URL (S3, ephemeral), `gpsLatitude`/`gpsLongitude` at recording time, `recordedAt`, `durationSeconds`, moderation decision fields. **Deleted within 2 hours of Requester acceptance** [PRD REVIEW window] or **within 24 hours of rejection/expiry** — this needs a scheduled job, not manual cleanup.
...
### `ConsentRecord`
Immutable. `userId`, `type` (ToS/Community Guidelines/Recording Policy/Privacy Policy/Requester Declaration/Creator Declaration), `timestamp`. Never update or delete rows — insert-only, for DPDP audit (PRD §9.1, §5.7.3).

### `AdminAuditLog`
Immutable, read-only for all roles. Every Admin/Moderator action: `actorId`, `action`, `targetEntityType`, `targetEntityId`, `timestamp`, `metadata?`.

### Extend existing models
- `User`: add `trustScoreBadge` (computed, `LOW`|`MEDIUM`|`HIGH`), KYC fields (`kycStatus`, `panNumber?`, `aadhaarNumber?` — only required past ₹50,000 cumulative annual payout), consecutive-rejection counter (drives the tutorial re-prompt, PRD §5.11b.3).
...
| AWS S3 (or equivalent) | Ephemeral video storage pending moderation/delivery | Not integrated — current Cloudinary use is profile images only |
| WebRTC / managed streaming service | Live in-app recording + upload | Not integrated |
...
- **Notifications** — FCM push + in-app feed, already has the token registration / read / read-all pattern the PRD's notification matrix (§8) needs. Needs many new trigger events wired (new request nearby, creator assigned, recording started, video ready, etc.) — infra is reusable, triggers are not built.
...
- Temporary chat + server-side content filtering
- Recording/upload pipeline (in-app camera only, GPS+timestamp metadata, minimum duration enforcement)
- Manual Moderation workflow (pre-publish + video review queues, GPS map view, reject reasons)
- Requester Review & Re-shoot workflow
- Mutual Ratings, Report/Abuse workflow with 3-strikes auto-suspend
...
- Dispute Center (Admin arbitration, partial-refund splits)
- Live Monitoring Dashboard, Active Request Dashboard, Moderation Queue (Admin views)
- Commission Settings (configurable %, audit-logged)
- Admin Audit Logs (immutable action log) — currently no audit trail exists at all, even for existing block/suspicious toggles
- Consent records for Recording Policy / Requester & Creator declarations (a generic ToS consent doesn't exist today either)
